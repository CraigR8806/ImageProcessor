...
class StandardConvolution(Convolution):
    ...
    @classmethod
    def randomKernel(cls, length, calcManager=None, random_low=0, random_high=1):
        kernels=[Kernel.random(length, calcManager=calcManager, random_low=random_low, random_high=random_high)]
        return cls(kernels)
